...
class SNN(nn.Module):
    def __init__(self, tau):
        ...
        self.layer = nn.Sequential(
            layer.Flatten(),
            layer.Linear(28*28, 10, bias=False), # Input
...
def main():
    ...
    parser.add_argument('-data-dir', type=str, default='./datasets', help='root dir of MNIST dataset')
    ...
    scaler = None
    if args.amp:
        scaler = amp.GradScaler() # Prevent underflow
    ...
    if args.opt == 'sgd':
        optimizer = torch.optim.SGD(net.parameters(), lr = args.lr, momentum=args.momemtum)
    elif args.opt == 'adam': # Default
    ...
    out_dir = os.path.join(args.out_dir, f'fc_T{args.T}_b{args.b}_{args.opt}_lr{args.lr}')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for img, label in train_data_loader:
            ...
            if scaler is not None: # Training with gradient scale (Mixed-precision training to prevent underflow)
                with amp.autocast():
                    out_fr = 0
                    for t in range(args.T):
                        encoded_img = encoder(img)
                        out_fr += net(encoded_img)
                    out_fr = out_fr/args.T # out_fr.shape = [batch_size, 10]. Firing rate output recorded
                    loss = F.mse_loss(out_fr, label_onehot) # Loss function: MSE between firing rate and true category
